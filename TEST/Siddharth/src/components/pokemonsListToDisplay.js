...
const limit = 20;
class PokemonListToDisplay extends Component{
constructor(props){
    super(props);
    this.state={
        data:undefined,
...
componentDidMount(){
     this.getPokemonList(Number(this.props.match.params.pageNumber));
}
...
async getPokemonList(pageNumber){
                ...
                <li key={pokemon.url.split("/")[6]}>
                   <Link to={'/pokemon/'+pokemon.url.split("/")[6]}>{pokemon.name}</Link>
                    ...
                    if(this.state.data){
                            ...
                            {
                                previous =  <Link to={'/pokemon/page/'+ Number(this.state.pageNumber) - 1}>Previous</Link>
                            }
                           if(this.state.data.next)
                            {
                                next = <Link to={'/pokemon/page/'+(Number(this.state.pageNumber) + 1)}>Next</Link>
                            }
                    }
...
export default PokemonListToDisplay;
